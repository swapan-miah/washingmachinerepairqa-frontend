...
export default function EditHeading({ data }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${process.env.BASE_URL}/section-heading/edit/${data._id}`, {
  ...
  return (
    <div className="p-5">
      <form onSubmit={handleSubmit} className="p-5 bg-white flex flex-col gap-5 border border-gray-200 rounded">
        ...
        <button type="submit" className="bg-[#0d9488] py-2 px-5 text-white font-medium rounded">
          Update
        </button>
      </form>
    </div>
