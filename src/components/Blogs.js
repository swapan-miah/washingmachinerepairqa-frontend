...
export default function Blogs({ showAll = false }) {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			setBlogs(blogsResponse.data.slice().reverse());
			setError(null);
		} catch (err) {
	...
	const getFirstTagText = useCallback((htmlString) => {
	...
	if (error)
		return (
			<div className="container mx-auto py-14 text-center text-red-600">
				<p>{error}</p>
			</div>
		);
		...
		<div className="bg-gray-50 overflow-x-hidden border-t border-gray-200">
			...
			<div className="container mx-auto py-14 overflow-x-hidden text-center">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
					{(showAll ? blogs : blogs.slice(0, 9)).map((blog) => (
						<div
							key={blog._id}
							data-aos={
								blog._id.charCodeAt(0) % 2 === 0 ? "fade-left" : "fade-right"
							}
