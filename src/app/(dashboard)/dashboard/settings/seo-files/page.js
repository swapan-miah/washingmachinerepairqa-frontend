...
export default function Page() {
	...
	return (
		<div className="p-5">
            ...
            {
                !seoFile && <div className="p-5 h-full f-full">
				...
				<div className="space-y-4 bg-white border border-gray-200 rounded p-5">
					...
					{seoFile?.xmlUrl && (
						...
						>
							...
							>
								XML File
								<span className="text-xl">{openIndex === "xml" ? "−" : "+"}</span>
							</h3>
							<div
								className={`grid transition-all duration-500 ease-in-out px-2 ${
									openIndex === "xml" ? "grid-rows-[1fr] py-3" : "grid-rows-[0fr] py-0"
								}`}
							>
					...
					{seoFile?.txtUrl && (
						...
						>
							...
							>
								TXT File
								<span className="text-xl">{openIndex === "txt" ? "−" : "+"}</span>
							</h3>
							<div
								className={`grid transition-all duration-500 ease-in-out px-2 ${
									openIndex === "txt" ? "grid-rows-[1fr] py-3" : "grid-rows-[0fr] py-0"
								}`}
							>
								<div className="overflow-hidden text-gray-600 text-sm">
									<Link
