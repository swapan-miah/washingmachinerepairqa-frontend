	...
	const fetchData = async () => {
		try {
			const res = await axios.get(
				`${process.env.NEXT_PUBLIC_BASE_URL}/our-solutions`,
			);
			setData(res.data);
		} catch (err) {
	...
	const handleDelete = async (id) => {
		...
		try {
			const res = await axios.delete(
				`${process.env.NEXT_PUBLIC_BASE_URL}/our-solutions/${id}`,
			);
					...
					<tbody className="bg-white">
						{[...data].reverse().map((sData) => (
							<tr
								...
								<td className="px-4 py-3 text-sm">
									<span className="line-clamp-2">{sData.description}</span>
								</td>
					...
					{[...data].reverse().map((sData) => (
						<div
							...
							<div className="flex justify-between gap-3 px-3 py-4">
								...
								<span className="text-md text-gray-900 line-clamp-2 break-all">
									{sData.description}
								</span>
							</div>
						</div>
